...
function checksExistsUserAccount(request, response, next) {
  ...
  if(!user){
    return response.status(404).json({
      error: "Should not be able to find a user with this username"
    });
  }
  request.user = user;
  next();
...
function checkExistsUserTodo(request, response, next) {
  ...
  if(!todo){
    return response.status(404).json({
      error: "Should not be able to find a todo with this id"
    });
  }
  request.todo = todo;
  next();
...
app.post('/users', (request, response) => {
  ...
  if(userExists){
    return response.status(400).json({
      error: "Should not be able to create a new user when username already exists"
    });
  }
  const user = {
    id: uuidv4(),
...
app.delete('/todos/:id', checksExistsUserAccount, checkExistsUserTodo, (request, response) => {
  const { user,todo: userTodo } = request;
  const todos = user.todos.filter(
    todo => todo.id != userTodo.id
  );
  user.todos = todos;
